...
const Register = () => {
  ...
  return (
    <div className={styles.register}>
        <h1 >Cadastre-se para gerenciar seus gastos!</h1>
        <form onSubmit={handleSubmit}>
          ...
          <label>
            ...
            <input type="password" name="confirmePassword" required placeholder="Confirme sua senha"
            value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
            />
          </label>
          <button className='btn'>Cadastrar</button>
          {error && <p className='error'>{error}</p>}
          <p>Já tem cadastro? <NavLink to="/login">Entre com sua conta aqui</NavLink></p>
        </form>
    </div>
  )
}
